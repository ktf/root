...
TArrowDS::TArrowDS(std::shared_ptr<arrow::Table> table, std::vector<std::string> const &columns)
   : fTable{table}, fColumnNames{columns}, fNSlots(1)
{
   // We want to allow people to specify which columns they
   // need so that we can think of upfront IO optimizations.
   auto filterWantedColumns = [&columns = fColumnNames, &table = fTable ]()
   {
      if (columns.empty()) {
   ...
   auto getRecordsFirstColumn = [&columnNames = fColumnNames, &table = fTable ]()
   {
      if (columnNames.empty()) {
         throw std::runtime_error("At least one column required");
      }
      auto name = columnNames[0];
   ...
   auto verifyColumnSize = [&table = fTable](std::shared_ptr<arrow::Column> column, int nRecords)
   {
   ...
   auto addColumnToGetterIndex = [&index = fGetterIndex](int columnId)
   {
   ...
   auto resetGetterIndex = [&index = fGetterIndex]() { index.clear(); };
...
void TArrowDS::SetNSlots(unsigned int nSlots)
{
   ...
   auto splitInEqualRanges = [&nSlots = fNSlots, &ranges = fEntryRanges ](int nRecords, unsigned int newNSlots)
   {
      ranges.clear();
      nSlots = newNSlots;
   ...
   auto getNRecords = [&table = fTable, &columns = fColumnNames ]()->int
   {
      auto index = table->schema()->GetFieldIndex(columns[0]);
      return table->column(index)->length();
   };
...
{
   auto findGetterIndex = [&index = fGetterIndex](unsigned int column)
   {
